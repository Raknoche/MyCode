...
load('C:\Program Files\MATLAB\R2012a\bin\LUXCode\Scratch\RichardKnoche\KrypCal_RD\KrypCal_2p21\S1aMethod\libNEST_CH3TModel\RecombPhysicsResult\ERBandPrediction\S1aS1b_To_ERBand_RZVoxels.mat')


%% Load in Sep2014 ER S1a/S1b

load('C:\Program Files\MATLAB\R2012a\bin\LUXCode\Scratch\RichardKnoche\KrypCal_RD\KrypCal_2p21\S1aMethod\libNEST_CH3TModel\RecombPhysicsResult\ERBandPrediction\Nov2014_S1aS1b.mat')
...
%             
%         %Make MCMC of predicted band for the voxel
%                 for counter=1:Kr_count_3D(i,j,k)/10;
%                      xx=50*rand(1); %pick random S1 value
%                      yy= log10((10.^(powerlaw_a.*xx.^powerlaw_b)).*(SE_size_Nov2014/SE_size_Sep2015)); %Get associate log10(S1/S2) mean from power law fit
%                      yy_sigma=abs(yy-log10((10.^(powerlaw_a_upper.*xx.^powerlaw_b_upper)).*(SE_size_Nov2014/SE_size_Sep2015)) )/1.28; %Get associate log10(S1/S2) gaussian width from power law fit
%                      MCMC_events=[MCMC_events; normrnd(yy,yy_sigma,1)];
%                      MCMC_events_x=[MCMC_events_x; xx];
%                end
%             end
%         end
%     end
...
for i=1:size(s1ab_z_means,2)
        ...
        %Make MCMC of predicted band for the voxel
                for counter=1:Kr_count_1D(i);
                     xx=50*rand(1); %pick random S1 value
                     yy= log10((10.^(powerlaw_a.*xx.^powerlaw_b)).*(SE_size_Sep2015/SE_size_Sep2015)); %Get associate log10(S1/S2) mean from power law fit
                     yy_sigma=abs(yy-log10((10.^(powerlaw_a_upper.*xx.^powerlaw_b_upper)).*(SE_size_Sep2015/SE_size_Sep2015)) )/1.28; %Get associate log10(S1/S2) gaussian width from power law fit
                     MCMC_events=[MCMC_events; normrnd(yy,yy_sigma,1)];
                     MCMC_events_x=[MCMC_events_x; xx];
               end
            end
...
s1_max=50;
   band_sigma=1.28; %90% confidence bounds
           ...
           ER_bins=ER_bin_size/2+s1_min:ER_bin_size:s1_max;
               ...
               MCMC_counts_ER_hist=ones(size(ER_bins));
...
%             temporary, for calculating correct E-lifetime   
        for ER_bin = (ER_bin_size/2)+s1_min:ER_bin_size:(s1_max); %HIST uses bin centers
            ...
            ER_fit_cut= inrange(MCMC_events_x,[ER_bin-ER_bin_size/2,ER_bin+ER_bin_size/2]);
      
            ER_band_hist=hist(MCMC_events(ER_fit_cut),s2_s1_bin);
            MCMC_counts_ER_hist(index_i)=sum(ER_band_hist);
           
            Fit_ER=fit(s2_s1_bin',ER_band_hist','gauss1');
             
            rms_fit=fit_ML_normal(MCMC_events(ER_fit_cut),s2_s1_bin);
            
            %band=confint(Fit_ER,.1);
            %ER_sigma(index_i)= Fit_ER.c1/sqrt(2);
            ER_sigma(index_i)= sqrt(rms_fit.sig2);
            ER_sigma_sigma(index_i)=ER_sigma(index_i)/sqrt(2*MCMC_counts_ER_hist(index_i));
         ...
         g=fittype('a*x^b');
         MCMC_ER_mean_power_fit=fit(ER_bins(ER_bins>2)',mean_ER_band(ER_bins>2)',g,'startpoint',[ 2.5 -.1]);
         MCMC_ER_mean_fit=MCMC_ER_mean_power_fit.a.*ER_bins.^(MCMC_ER_mean_power_fit.b);
            b=confint(MCMC_ER_mean_power_fit,.68);
            MCMC_sigma_a_ER_mean=(b(2,1)-b(1,1))/2;
            MCMC_sigma_b_ER_mean=(b(2,2)-b(1,2))/2;
            
         MCMC_ER_lower_power_fit=fit(ER_bins(ER_bins>2)',lower_bound(ER_bins>2)',g,'startpoint',[ 2.5 -.1]);
         MCMC_ER_lower_fit=MCMC_ER_lower_power_fit.a.*ER_bins.^(MCMC_ER_lower_power_fit.b);
            b=confint(MCMC_ER_lower_power_fit,.68);
            MCMC_sigma_a_ER_lower=(b(2,1)-b(1,1))/2;
            MCMC_sigma_b_ER_lower=(b(2,2)-b(1,2))/2;
            
         MCMC_ER_upper_power_fit=fit(ER_bins(ER_bins>2)',upper_bound(ER_bins>2)',g,'startpoint',[ 2.5 -.1]);
         MCMC_ER_upper_fit=MCMC_ER_upper_power_fit.a.*ER_bins.^(MCMC_ER_upper_power_fit.b);
            b=confint(MCMC_ER_upper_power_fit,.68);
            MCMC_sigma_a_ER_upper=(b(2,1)-b(1,1))/2;
            MCMC_sigma_b_ER_upper=(b(2,2)-b(1,2))/2;

        MCMC_powerlaw_a=MCMC_ER_mean_power_fit.a;
        MCMC_powerlaw_b=MCMC_ER_mean_power_fit.b;
        MCMC_powerlaw_a_upper=MCMC_ER_upper_power_fit.a;
        MCMC_powerlaw_b_upper=MCMC_ER_upper_power_fit.b;
        MCMC_powerlaw_a_lower=MCMC_ER_lower_power_fit.a;
        MCMC_powerlaw_b_lower=MCMC_ER_lower_power_fit.b;
        MCMC_powerlaw_a_err=MCMC_sigma_a_ER_mean;
        MCMC_powerlaw_b_err=MCMC_sigma_b_ER_mean;
        MCMC_powerlaw_a_upper_err=MCMC_sigma_a_ER_upper;
        MCMC_powerlaw_b_upper_err=MCMC_sigma_b_ER_upper;
        MCMC_powerlaw_a_lower_err=MCMC_sigma_a_ER_lower;
        MCMC_powerlaw_b_lower_err=MCMC_sigma_b_ER_lower;
        
  %% Compare to actual band

load('C:\Program Files\MATLAB\R2012a\bin\LUXCode\Scratch\RichardKnoche\KrypCal_RD\KrypCal_2p21\S1aMethod\libNEST_CH3TModel\RecombPhysicsResult\ERBandPrediction\Nov2014_ERFit.mat')

  %Make Plot    

 no_fid_new=figure;
 hold on
 plot(MCMC_events_x,MCMC_events,'.k','MarkerSize',1)
 cc=colorbar;
...
plot(ER_bins,MCMC_ER_mean_fit,'-b','markersize',20,'linewidth',2)
...
plot(ER_bins,MCMC_ER_lower_fit,'--b','markersize',20,'linewidth',2)
plot(ER_bins,MCMC_ER_upper_fit,'--b','markersize',20,'linewidth',2)
  legend('Data','MCMC Band','Measured ER Band');

 
     bin_step=2;

            total_count=length(log10(MCMC_events_x));
...
%   title('Nov2014 CH3T')
  ...
  text(5,.55,strcat('+',num2str(band_sigma),'\sigma = (',num2str(MCMC_ER_upper_power_fit.a,3),'\pm', num2str(MCMC_sigma_a_ER_upper,1) ,')\times S1_c ^{', num2str(MCMC_ER_upper_power_fit.b,3),'\pm',num2str(MCMC_sigma_b_ER_upper,1),'}'),'fontsize',16,'interpreter','tex');
  text(5,.3,strcat('Mean=(',num2str(MCMC_ER_mean_power_fit.a,3),'\pm', num2str(MCMC_sigma_a_ER_mean,1),')\times S1_c ^{', num2str(MCMC_ER_mean_power_fit.b,3),'\pm', num2str(MCMC_sigma_b_ER_mean,1),'}'),'fontsize',16,'interpreter','tex');
  text(5,.05,strcat('-',num2str(band_sigma),'\sigma = (',num2str(MCMC_ER_lower_power_fit.a,3),'\pm', num2str(MCMC_sigma_a_ER_lower,1),')\times S1_c ^{', num2str(MCMC_ER_lower_power_fit.b,3),'\pm', num2str(MCMC_sigma_b_ER_lower,1),'}'),'fontsize',16,'interpreter','tex');
  title('Nov2014 CH3T')
...
band_mean_mc=MCMC_ER_mean_power_fit.a.*x.^MCMC_ER_mean_power_fit.b;
band_upper_mc=MCMC_ER_upper_power_fit.a*x.^MCMC_ER_upper_power_fit.b;
band_lower_mc=MCMC_ER_lower_power_fit.a*x.^MCMC_ER_lower_power_fit.b;
