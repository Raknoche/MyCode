...
path='C:\Users\Richard\Desktop\LUX Analysis\lux10_20160208T0930_cp19837';
...
%         & (s1a_phe_both./s1b_phe_both > 0.01.* s1a_phe_both) & (s1a_phe_both./s1b_phe_both<4.2) & (s1a_phe_both<255);
   ...
   %fidicual Z cut for s1ab_xy measurement
    ...
    dT_step=30; %2000 events per z bin
        ...
        for j = (-r_max+s1ab_xyz_ystep):s1ab_xyz_ystep:r_max %map columns across (x_cm) -- s1y bins are the max edge of the bin
           ...
           q = s1ab_x<j & s1ab_x>(j-s1ab_xyz_xstep) & s1ab_y<i & s1ab_y>(i-s1ab_xyz_ystep) & inrange(s1ab_z,[(k-s1ab_xyz_zstep/2) , (k+s1ab_xyz_zstep/2)]); %no 0th element!
            ...
            if (Count_S1ab_3D(l,m,n) >= 100) % at least 100 counts before fitting. 
                ...
                s1a_xyz_mean_vox{voxel_counter}=s1a_z_fit.b1;
                s1a_xyz_mean_err_vox{voxel_counter}=s1a_z_fit.c1/sqrt(2)/length(s1a_phe_both(q.' & inrange(s1a_phe_both,[0 400])));
                ...
                s1b_xyz_mean_vox{voxel_counter}=s1b_z_fit.b1;
                s1b_xyz_mean_err_vox{voxel_counter}=s1b_z_fit.c1/sqrt(2)/length(s1b_phe_both(q.' & inrange(s1b_phe_both,[0 300])));
                ...
                s1a_xyz_mean_vox{voxel_counter}=0;
                s1a_xyz_mean_err_vox{voxel_counter}=0;
                s1b_xyz_mean_vox{voxel_counter}=0;
                s1b_xyz_mean_err_vox{voxel_counter}=0;                
            end
...
k
     ...
     s1ab_xyz_mean_vox=s1a_xyz_mean_vox./s1b_xyz_mean_vox;
     s1ab_xyz_mean_err_vox=sqrt( (s1b_xyz_mean_err_vox.*s1a_xyz_mean_vox./(s1b_xyz_mean_vox.^2)).^2 + (s1a_xyz_mean_err_vox./s1b_xyz_mean_vox).^2);
      
  %Save ER Band fits
  save('Feb2016_S1aS1b_Vox','s1ab_xyz_mean_vox','s1ab_xyz_mean_err_vox','s1ab_xyz_mean','s1ab_xyz_xbins','s1ab_xyz_ybins','s1ab_xyz_zbins','s1ab_P','s1ab_z_means','s1ab_bincenters','Kr_count_3D','Kr_count_1D');

  s1ab_xyz_xbins_old=s1ab_xyz_xbins;
  s1ab_xyz_ybins_old=s1ab_xyz_ybins;
  s1ab_xyz_zbins_old=s1ab_xyz_zbins;
  s1ab_bincenters_old=s1ab_bincenters;
  s1ab_xyz_zstep_old=s1ab_xyz_zstep;
  s1ab_xyz_xstep_old=s1ab_xyz_xstep;
  s1ab_xyz_ystep_old=s1ab_xyz_ystep;
  r_max_old=r_max;
  dT_step_old=dT_step;
  s1ab_z_cut_upper_old=s1ab_z_cut_upper;
    s1ab_z_old = s1ab_z;

  clearvars -except s1ab_z_old s1ab_xyz_xbins_old s1ab_xyz_ybins_old s1ab_xyz_zbins_old s1ab_bincenters_old s1ab_xyz_zstep_old s1ab_xyz_xstep_old s1ab_xyz_ystep_old r_max_old dT_step_old s1ab_z_cut_upper_old
   ...
   %Save ER Band fits
  save('Feb2016_S1aS1b_Vox','s1ab_P','s1ab_z_means','s1ab_bincenters','Kr_count_1D');
...
%% Make corrections with the chosen polynomial
load('C:\Program Files\MATLAB\R2012a\bin\LUXCode\Scratch\RichardKnoche\KrypCal_RD\KrypCal_2p22\KrypCal2p22_QualityChecks\ERBand_Prediction\MappingParametersStart_Feb2016')
...
load('C:\Program Files\MATLAB\R2012a\bin\CH3T_Feb2016_Kr2p18.mat');
...
pulse_area_cut= (log10(s1_phe_both_xyz)+0.7*log10(s2_phe_both_xyz)<4.8) & (log10(s1_phe_both_xyz)+0.7*log10(s2_phe_both_xyz)>2.0) & ...
...
%% Get ERBand In the Same Voxels - Zdep

voxel_counter=1;
for z_max=10+dT_step_old:dT_step_old:s1ab_z_cut_upper_old
            
fiducial_cut=inrange(s2radius_del,[0,21]) & inrange(drift_time,[z_max-dT_step_old z_max]);
...
%             temporary, for calculating correct E-lifetime   
  ...
  z_center_vox{voxel_counter}=z_max-dT_step_old/2;
   
  voxel_counter=voxel_counter+1;
end
    

    save('Feb2016_ER_Vox','ER_lower_power_fit_vox','ER_upper_power_fit_vox','ER_mean_power_fit_vox','z_center_vox')
    

%% Get ERBand In the Same Voxels - 3D dep

voxel_counter=1;
if length(s1ab_z_old)>100000; %require 100,000 events to do 3D binning

for k = s1ab_xyz_zbins; %s1zbins are the center of the bin    
    for i = (-r_max+s1ab_xyz_xstep):s1ab_xyz_xstep_old:r_max_old %map to rows going down (y_cm) -- s1x bins are the max edge of the bin
        for j = (-r_max+s1ab_xyz_ystep):s1ab_xyz_ystep_old:r_max_old %map columns across (x_cm) -- s1y bins are the max edge of the bin

fiducial_cut=inrange(drift_time,[40 300]) & ...
    s2x<j & s2x>(j-s1ab_xyz_xstep_old) & s2y<i & s2y>(i-s1ab_xyz_ystep_old) & inrange(drift_time,[(k-s1ab_xyz_zstep_old/2) , (k+s1ab_xyz_zstep_old/2)])
...
%             temporary, for calculating correct E-lifetime   
  ...
  ER_lower_power_fit_3Dvox{voxel_counter}=ER_lower_power_fit;
  ER_upper_power_fit_3Dvox{voxel_counter}=ER_upper_power_fit;
  ER_mean_power_fit_3Dvox{voxel_counter}=ER_mean_power_fit;
  x_center_3Dvox{voxel_counter}=j-s1ab_xyz_xstep_old/2;
  y_center_3Dvox{voxel_counter}=i-s1ab_xyz_ystep_old/2;
  z_center_3Dvox{voxel_counter}=k;
  ...
  voxel_counter=voxel_counter+1;
    ...
    save('Feb2016_ER_Vox','ER_lower_power_fit_vox','ER_upper_power_fit_vox','ER_mean_power_fit_vox','z_center_vox',...
        'ER_lower_power_fit_3Dvox','ER_upper_power_fit_3Dvox','ER_mean_power_fit_3Dvox','x_center_3Dvox','y_center_3Dvox','z_center_3Dvox')
