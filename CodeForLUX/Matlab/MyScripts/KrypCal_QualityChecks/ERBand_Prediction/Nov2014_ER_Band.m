...
load('MappingParametersStart_Nov2014')
...
%Removing zeros from map with cubic neighbor method
...
%     bin_size = (488/bins);%span the entire detector. ~49.0 cm. Bin centers every 4\mus
     bin_size = (floor(det_edge/bins*4)/4);%span the entire detector. 325us. typically = 5us. Round to cubic 0.25.
...
%  plot(ER_bins,upper_bound,'ok','markersize',6,'linewidth',2)
 ...
 box;
  ...
  %Save ER Band fits
  save('Nov2014_ERFit','ER_bins','mean_ER_band','ER_sigma','ER_lower_power_fit','ER_upper_power_fit','ER_mean_power_fit','ER_lower_fit','ER_upper_fit','ER_mean_fit');
