...
Kr_count_2D=zeros(length(s1ab_zbins),length(s1ab_r2bins));
 ...
 for r2_bin_max=s1ab_r2bin_min+s1ab_r2_binsize:s1ab_r2_binsize:s1ab_r2bin_max;
     z_counter=1;
        for z_bin_max=s1ab_zbin_min+s1ab_z_binsize:s1ab_z_binsize:s1ab_zbin_max;          
          ...
          kr_cut =inrange(s2radius.^2,[r2_bin_max-s1ab_r2_binsize,r2_bin_max]).' & inrange(drift_time,[z_bin_max-s1ab_z_binsize,z_bin_max]).'; 
          
          Kr_count_2D(z_counter,r2_counter)=length(s1_phe_both(kr_cut));
...
if length(s1ab_z)>90000; %require 100,000 events to do 3D binning - CHANGED TO 90k TO FORCE THIS TO HAPPEN FOR TESTING
...
k
  ...
  %Save ER Band fits
  save('Sep2015_S1aS1b','s1ab_r2z_mean','Kr_count_2D','s1ab_xyz_mean','s1ab_xyz_xbins','s1ab_xyz_ybins','s1ab_xyz_zbins','s1ab_P','s1ab_z_means','s1ab_bincenters','Kr_count_3D','Kr_count_1D');

else
   %Save ER Band fits
  save('Sep2015_S1aS1b','s1ab_r2z_mean','Kr_count_2D','s1ab_P','s1ab_z_means','s1ab_bincenters','Kr_count_1D');
