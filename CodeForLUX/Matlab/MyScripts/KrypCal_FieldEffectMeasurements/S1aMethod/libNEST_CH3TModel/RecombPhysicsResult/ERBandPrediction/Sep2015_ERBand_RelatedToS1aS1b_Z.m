...
%Set up cuts and variables
   ...
   %fidicual Z cut for s1ab_xy measurement
    ...
    dT_step=det_edge/(length(s1ab_z)/2000); %2000 events per z bin
...
s1_phe_both=s1_phe_both./(s1_zfieldremoval.*s1_xyfieldremoval);
 ...
 figure
 plot(drift_time,(s1_zfieldremoval.*s1_xyfieldremoval),'.k')
 hold on;
...
voxel_counter=1;
  ...
  ER_mean_power_fit_vox={};
    ...
    h3_r_cut=(drift_time-rslope.*s2radius)<rintercept;    
...
fiducial_cut=h3_r_cut & inrange(drift_time,[z_max-s1ab_z_binsize z_max]);  
tritium_cut= clean_cut & pulse_area_cut & fiducial_cut;

if length(s1_phe_both_xyz(tritium_cut))>5000
...
%             temporary, for calculating correct E-lifetime   
  ...
  z_centers(voxel_counter)=z_max-dT_step/2;
    skipflag(voxel_counter)=0;

else
  ER_lower_power_fit_vox{voxel_counter}=0;
  ER_upper_power_fit_vox{voxel_counter}=0;
  ER_mean_power_fit_vox{voxel_counter}=0;
  
  ER_lower_power_fit_vox_aerr{voxel_counter}=0;
  ER_upper_power_fit_vox_aerr{voxel_counter}=0;
  ER_mean_power_fit_vox_aerr{voxel_counter}=0;

  ER_lower_power_fit_vox_berr{voxel_counter}=0;
  ER_upper_power_fit_vox_berr{voxel_counter}=0;
  ER_mean_power_fit_vox_berr{voxel_counter}=0;
  
  z_centers(voxel_counter)=0;
  
  skipflag(voxel_counter)=1;
end
...
%Relating parameter a to S1aS1b
temp_mean_s1as1b_map=s1ab_z_means(:);
temp_mean_s1as1b_map_totalerr=s1ab_z_means_err(:);
...
temp_ER_mean_a(i)=0;
temp_ER_mean_a_err(i)=0;

temp_ER_mean_b(i)=0;
temp_ER_mean_b_err(i)=0;

temp_ER_lower_a(i)=0;
temp_ER_lower_a_err(i)=0;

temp_ER_lower_b(i)=0;
temp_ER_lower_b_err(i)=0;

temp_ER_upper_a(i)=0;
temp_ER_upper_a_err(i)=0;

temp_ER_upper_b(i)=0;
temp_ER_upper_b_err(i)=0;     
    end
end

nancut=~isnan(temp_mean_s1as1b_map).' & ~isnan(temp_ER_mean_a) & temp_ER_mean_a~=0;

[S1aS1b_to_mean_a_fit S1aS1b_to_mean_a_fit_err]=polyfit(temp_mean_s1as1b_map(nancut),temp_ER_mean_a(nancut).',2);
[S1aS1b_to_mean_b_fit S1aS1b_to_mean_b_fit_err]=polyfit(temp_mean_s1as1b_map(nancut),temp_ER_mean_b(nancut).',2);
[S1aS1b_to_lower_a_fit S1aS1b_to_lower_a_fit_err]=polyfit(temp_mean_s1as1b_map(nancut),temp_ER_lower_a(nancut).',2);
[S1aS1b_to_lower_b_fit S1aS1b_to_lower_b_fit_err]=polyfit(temp_mean_s1as1b_map(nancut),temp_ER_lower_b(nancut).',2);
[S1aS1b_to_upper_a_fit S1aS1b_to_upper_a_fit_err]=polyfit(temp_mean_s1as1b_map(nancut),temp_ER_upper_a(nancut).',2);
[S1aS1b_to_upper_b_fit S1aS1b_to_upper_b_fit_err]=polyfit(temp_mean_s1as1b_map(nancut),temp_ER_upper_b(nancut).',2);

%% Plot coefficient relationship
figure
hold on;
xlabel('S1a/S1b'); ylabel('Power Law Coefficient');
myfigview(16);

[tempy tempyerr]=polyval(S1aS1b_to_mean_a_fit,[0:0.005:4],S1aS1b_to_mean_a_fit_err);
count=1;
clear tempfill
for i=length(tempy):-1:1;
    tempfill(count)=tempy(i)-tempyerr(i);
    count=count+1;
end
fill([0:.005:4,4:-0.005:0],[tempy+tempyerr,tempfill],[0.6 0.6 1],'EdgeColor',[0.6 0.6 1])
plot([0:0.005:4],tempy,'-b','LineWidth',2);
xlim([2.3 3])
rkploterr(temp_mean_s1as1b_map(nancut),temp_ER_mean_a(nancut),temp_mean_s1as1b_map_totalerr(nancut),temp_ER_mean_a_err(nancut),[0 0 0],[],[],2)

%% Plot exponent relationship
figure
hold on;
xlabel('S1a/S1b'); ylabel('Power Law Exponent');
myfigview(16);

clear tempy tempyerr tempfill
[tempy tempyerr]=polyval(S1aS1b_to_mean_b_fit,[0:0.005:4],S1aS1b_to_mean_b_fit_err);
count=1;
for i=length(tempy):-1:1;
    tempfill(count)=tempy(i)-tempyerr(i);
    count=count+1;
...
plot([0:0.005:4],tempy,'-b','LineWidth',2);
xlim([2.3 3])
rkploterr(temp_mean_s1as1b_map(nancut),temp_ER_mean_b(nancut),temp_mean_s1as1b_map_totalerr(nancut),temp_ER_mean_b_err(nancut),[0 0 0],[],[],2)


save('S1aS1b_To_ERBand_ZVoxels','S1aS1b_to_mean_a_fit','S1aS1b_to_mean_b_fit','S1aS1b_to_lower_a_fit','S1aS1b_to_lower_b_fit','S1aS1b_to_upper_a_fit','S1aS1b_to_upper_b_fit',...
    'S1aS1b_to_mean_a_fit_err','S1aS1b_to_mean_b_fit_err','S1aS1b_to_lower_a_fit_err','S1aS1b_to_lower_b_fit_err','S1aS1b_to_upper_a_fit_err','S1aS1b_to_upper_b_fit_err');