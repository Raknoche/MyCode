...
% norm_s1_both_xyz=cell(122,1); % CHANGE THIS TO WORK WITH GROUPS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tic
...
% 
% center_phe_both_xyz=interp3(s1xbins,s1ybins,s1zbins,Mean_S1_3D,0,0,(det_edge-4)/2,'spline');%Normalize to the center (x=y=0. dT=160us)
center_phe_both_xyz=summed_center;

Mean_S1_3D(Mean_S1_3D==0)=nan;  
Mean_S1_3D=inpaint_nans3(Mean_S1_3D);

norm_s1=center_phe_both_xyz./Mean_S1_3D; %Normalize to center (x=y=0. dT=160)
...
normalized_s1=s1.*interp3(s1xbins,s1ybins,s1zbins,norm_s1,xc,yc,dT,'spline');
...
temp_energy_est=summed_center*normalized_s1./interp3(s1xbins,s1ybins,s1zbins,Mean_S1_3D,xc,yc,dT,'spline'); 
...
s1_corrected(i)=sum(temp_energy_estimate./temp_variance)/sum(1./temp_variance); %should peak at summed_center keV
end

%%% SAVE WORKSPACE %%%%%%%
% disp('Saving Workspace');
% pause(0.1);
% save('Latest_S1_PMTxPMT_v1_TestingWeights');
% pause(0.1);
