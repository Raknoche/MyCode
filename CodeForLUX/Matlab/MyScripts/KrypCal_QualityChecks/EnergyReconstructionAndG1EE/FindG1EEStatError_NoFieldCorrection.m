...
kr_2015_chi2_slices=((41.55-kr_2015_means)./0.2395).^2;
      ...
      %make CH3T data spectrum
        ...
        h3_chi2_2015=(1/62).*sum( (h3_scaled_oldsim_2015(17:80) - h3_hist_2015(17:80)).^2 ./(h3_hist_2015(17:80)));    
                ...
                total_h3_chi2_list(counter)=(h3_chi2_2016+h3_chi2_2015)/2;
                total_kr_chi2_list(counter)=(mean(kr_2016_chi2_slices)+mean(kr_2015_chi2_slices))/2;
                total_chi2_list(counter)=(total_h3_chi2+total_kr_chi2)/2;
                ee_list(counter)=ee;
                g1_list(counter)=g1;
...
ee_cut=total_chi2_list>0  &  g1_list==bestg1;


% Find 1.4 chi2 diff = 0.68 interval = 1 sigma interval
EErange=ee_list(ee_cut);
Chi2range=total_chi2_list(ee_cut);

EEupperbound=max(EErange(Chi2range-min(Chi2range) < 1.4));
EE_one_sig=EEupperbound-bestEE

%% Repeat for g1
g1_cut=total_chi2_list>0 & ee_list==bestEE;

% Find 1.4 chi2 diff = 0.68 interval = 1 sigma interval
g1range=g1_list(g1_cut);
Chi2range=total_chi2_list(g1_cut);

g1upperbound=max(g1range(Chi2range-min(Chi2range) < 1.4));
g1_one_sig=g1upperbound-bestg1
