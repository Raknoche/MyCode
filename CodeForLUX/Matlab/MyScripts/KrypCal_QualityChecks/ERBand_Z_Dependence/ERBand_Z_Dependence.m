...
pulse_area_cut= (log10(s1_phe_both_xyz)+0.7*log10(s2_phe_both_xyz)<4.85) & (log10(s1_phe_both_xyz)+0.7*log10(s2_phe_both_xyz)>2) & ...
    (log10(s1_phe_both_xyz)-2*log10(s2_phe_both_xyz)<-5) & (log10(s1_phe_both_xyz)-2*log10(s2_phe_both_xyz)>-7);
...
bin_counter=1;
for dT_max=10+dT_step:dT_step:300;
    
fiducial_cut=inrange(drift_time,[dT_max-dT_step, dT_max]) & inrange(s2radius_del,[0,max_r]);
...
%             temporary, for calculating correct E-lifetime   
 ...
 color_counter=1;
 for i=[1,5,9];
 plot(ER_bins,ER_lower_fit{i},'--','Color',colors{color_counter},'markersize',14,'linewidth',2)
 plot(ER_bins,ER_mean_fit{i},'-','Color',colors{color_counter},'markersize',20,'linewidth',2)
 plot(ER_bins,ER_upper_fit{i},'--','Color',colors{color_counter},'markersize',14,'linewidth',2)
 color_counter=color_counter+1;
 end
 
 save('ERBand_Z_Dep_WithFieldCorr','ER_bins','ER_lower_fit','ER_mean_fit','ER_upper_fit');
