...
path='C:\Users\Richard\Desktop\LUX Analysis\MultiZDD_KrData\lux10_20140920T1613_cp13946';
...
%Hard coded numbers from energy chi2
...
for NR_bin = (NR_bin_size/2)+s1_min:NR_bin_size:(s1_max); %HIST uses bin centers
    ...
    NR_fit_cut=DD_cut & inrange(s1_phe_both_xyz,[NR_bin-NR_bin_size/2,NR_bin+NR_bin_size/2]);

    NR_band_hist=hist(log10(s2_phe_both_xyz(NR_fit_cut)./s1_phe_both_xyz(NR_fit_cut)),s2_s1_bin);
    counts_NR_hist(index_i)=sum(NR_band_hist);

    Fit_NR=fit(s2_s1_bin',NR_band_hist','gauss1');

    rms_fit=fit_ML_normal(log10(s2_phe_both_xyz(NR_fit_cut)./s1_phe_both_xyz(NR_fit_cut)),s2_s1_bin);
...
g=fittype('a*x^b');
NR_mean_power_fit=fit(NR_bins(NR_bins>2)',mean_NR_band(NR_bins>2)',g,'startpoint',[ 2.5 -.1]);
NR_mean_fit{bin_counter}=NR_mean_power_fit.a.*NR_bins.^(NR_mean_power_fit.b);

NR_lower_power_fit=fit(NR_bins(NR_bins>2)',lower_bound(NR_bins>2)',g,'startpoint',[ 2.5 -.1]);
NR_lower_fit{bin_counter}=NR_lower_power_fit.a.*NR_bins.^(NR_lower_power_fit.b);

NR_upper_power_fit=fit(NR_bins(NR_bins>2)',upper_bound(NR_bins>2)',g,'startpoint',[ 2.5 -.1]);
NR_upper_fit{bin_counter}=NR_upper_power_fit.a.*NR_bins.^(NR_upper_power_fit.b);

NR_mean_fit_points{bin_counter}=mean_NR_band;
NR_lower_fit_points{bin_counter}=lower_bound;
NR_upper_fit_points{bin_counter}=upper_bound;

NR_mean_fit_points_err{bin_counter}=mean_NR_band_err;
NR_lower_fit_points_err{bin_counter}=lower_bound_err;
NR_upper_fit_points_err{bin_counter}=upper_bound_err;

NR_bins_tracked{bin_counter}=NR_bins;
...
%          bin_counter=bin_counter+1;
 ...
 color_counter=1;
 for i=[1:1:5];
 plot(NR_bins_tracked{i},NR_lower_fit{i},'--','Color',colors{color_counter},'markersize',14,'linewidth',2)
 plot(NR_bins_tracked{i},NR_mean_fit{i},'-','Color',colors{color_counter},'markersize',20,'linewidth',2)
 plot(NR_bins_tracked{i},NR_upper_fit{i},'--','Color',colors{color_counter},'markersize',14,'linewidth',2)
 
%  tempx=NR_bins_tracked{i};
%  tempy=NR_mean_fit_points{i};
%  tempy_err=NR_mean_fit_points_err{i};
%  
%  if i ~= 4
%  errorbar(tempx(2:end-1),tempy(2:end-1),tempy_err(2:end-1),'Color',colors{color_counter})
%  else
%  errorbar(tempx(2:end-2),tempy(2:end-2),tempy_err(2:end-2),'Color',colors{color_counter})
%  end     
%   
...
pulse_area_cut= (log10(s1_phe_both)+0.7*log10(s2_phe_both)<4.85) & (log10(s1_phe_both)+0.7*log10(s2_phe_both)>2) & ...
    (log10(s1_phe_both)-2*log10(s2_phe_both)<-4.7) & (log10(s1_phe_both)-2*log10(s2_phe_both)>-6.7);
fiducial_cut=inrange(drift_time,[35 75]) & inrange(s2radius_del,[0,max_r]);

tritium_cut= pulse_area_cut  & fiducial_cut;
...
%             ER_sigma(index_i)= Fit_ER.c1/sqrt(2);
         ...
         temp_NR_mean=NR_mean_fit_points{bin_counter};


              %Refit the NR band - ER binning is same as NR binning
             NR_mean_data=NR_mean_fit{bin_counter};
             NR_bin_data=NR_bins_tracked{bin_counter};
             NR_mean_refit=fit(NR_bin_data(NR_bin_data>2)',NR_mean_data(NR_bin_data>2)',g,'startpoint',[ 2.5 -.1]);
             NR_mean_line=NR_mean_refit.a.*ER_bins.^(NR_mean_refit.b);
  
             
                     
             total_count=length(s1_phe_both_xyz(tritium_cut & s1_phe_both_xyz<50));
             leakage_count=length(s1_phe_both_xyz(tritium_cut & s1_phe_both_xyz<50 & (log10(s2_phe_both_xyz./s1_phe_both_xyz) < NR_mean_refit.a.*s1_phe_both_xyz.^(NR_mean_refit.b))));
             average_leakage(bin_counter)=leakage_count./total_count;
             average_leakage_error(bin_counter)=sqrt( (sqrt(leakage_count)./total_count).^2 + (leakage_count.*sqrt(total_count)./(total_count.^2)).^2);
             
             figure
             plot(s1_phe_both_xyz(tritium_cut),log10(s2_phe_both_xyz(tritium_cut)./s1_phe_both_xyz(tritium_cut)),'.k')
             hold on;
             plot(ER_bins,ER_mean_fit{bin_counter},'-b');
             plot(ER_bins,ER_lower_fit{bin_counter},'--b');
             plot(ER_bins,ER_upper_fit{bin_counter},'--b');
             plot(ER_bins,NR_mean_line,'-r');
             xlim([0 50])
             title('Drift time 35-85');
             xlabel('S1'); ylabel('Log10(S2/S1)'); myfigview(16);
...
%             ER_sigma(index_i)= Fit_ER.c1/sqrt(2);
         ...
         temp_NR_mean=NR_mean_fit_points{bin_counter};
             ...
             plot(ER_bins,NR_mean_line,'-r');
             xlim([0 50])
             title('Drift time 235-280');
             xlabel('S1'); ylabel('Log10(S2/S1)'); myfigview(16);
             
%%
figure
average_drift=[(35+75)/2,(75+125)/2,(125+175)/2,(180+225)/2,(235+280)/2];
errorbar(average_drift,average_leakage,average_leakage_error,'.k')
xlabel('Average Drift Time (uSec)'); ylabel('Leakage Fraction');
myfigview(16);


save('AverageLeakage','average_drift','average_leakage','average_leakage_error');